...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import HelloCExecutable_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"HelloCExecutable"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("HelloCExecutable_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "HelloCExecutable_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"HelloCExecutable", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "HelloCExecutable_jll", "src", "HelloCExecutable_jll.jl"))
        @info("HelloCExecutable_ll dev'ed out to /Users/terasaki/.julia/dev/HelloCExecutable_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("58f04f38-d89e-574d-ac21-80cb6e819e9f"))
platforms = [Pkg.Artifacts.unpack_platform(e, "HelloCExecutable", artifacts_toml) for e in artifacts["HelloCExecutable"]]
...
if best_platform === nothing
    @debug("Unable to load HelloCExecutable; unsupported platform $(triplet(platform_key_abi()))")
else
...
end  # module HelloCExecutable_jll
